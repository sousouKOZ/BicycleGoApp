import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  static const seed = Color(0xFF00A88F);

  static const background = Color(0xFFF6F8F5);
  static const surface = Color(0xFFFFFFFF);

  static const onSurfacePrimary = Color(0xFF1A1F36);
  static const onSurfaceSecondary = Color(0xFF5A6278);

  static const accent = Color(0xFF00A88F);
  static const accentAlt = Color(0xFF2E7CF6);
  static const navigation = Color(0xFF2E7CF6);
  static const coupon = Color(0xFFF4A43A);
  static const success = Color(0xFF18B27A);
  static const warning = Color(0xFFF4A43A);
  static const danger = Color(0xFFE5484D);

  static const glassBorder = Color(0x33FFFFFF);
  static const glassShadow = Color(0x14000000);

  // Dark theme
  static const backgroundDark = Color(0xFF0D1413);
  static const surfaceDark = Color(0xFF17211F);
  static const onSurfacePrimaryDark = Color(0xFFF5F7FB);
  static const onSurfaceSecondaryDark = Color(0xFFAAB1C2);
  static const glassBorderDark = Color(0x33FFFFFF);
  static const glassShadowDark = Color(0x44000000);
}

extension AppTextColors on BuildContext {
  Color get textPrimary => Theme.of(this).brightness == Brightness.dark
      ? AppColors.onSurfacePrimaryDark
      : AppColors.onSurfacePrimary;

  Color get textSecondary => Theme.of(this).brightness == Brightness.dark
      ? AppColors.onSurfaceSecondaryDark
      : AppColors.onSurfaceSecondary;

  Color get subtleBorder => Theme.of(this).brightness == Brightness.dark
      ? AppColors.onSurfaceSecondaryDark.withValues(alpha: 0.2)
      : AppColors.onSurfaceSecondary.withValues(alpha: 0.12);
}
