import 'package:flutter/material.dart';

import 'app_colors.dart';

/// BackdropFilterを使わない"ガラス風"のBoxDecoration集。
/// 半透明塗り + 細い白縁 + 柔らかい影で軽量にガラス感を再現する。
/// context を渡すとダークモードに追従する。
class GlassDecoration {
  GlassDecoration._();

  static bool _isDark(BuildContext? context) =>
      context != null && Theme.of(context).brightness == Brightness.dark;

  /// 明るい地（地図の上、白背景の上）で使う標準ガラス。
  static BoxDecoration light(
    BuildContext? context, {
    double radius = 20,
    double opacity = 0.88,
  }) {
    final dark = _isDark(context);
    return BoxDecoration(
      color: (dark ? AppColors.surfaceDark : AppColors.surface)
          .withValues(alpha: opacity),
      borderRadius: BorderRadius.circular(radius),
      border: Border.all(
        color: (dark ? Colors.white : Colors.white).withValues(
          alpha: dark ? 0.08 : 0.6,
        ),
        width: 1,
      ),
      boxShadow: [
        BoxShadow(
          color: dark
              ? Colors.black.withValues(alpha: 0.35)
              : const Color(0x1A1A1F36),
          blurRadius: 24,
          spreadRadius: -8,
          offset: const Offset(0, 10),
        ),
      ],
    );
  }

  /// ピル型（丸括弧状のトグル、チップなど）。
  static BoxDecoration pill(BuildContext? context, {double opacity = 0.92}) {
    final dark = _isDark(context);
    return BoxDecoration(
      color: (dark ? AppColors.surfaceDark : AppColors.surface)
          .withValues(alpha: opacity),
      borderRadius: BorderRadius.circular(999),
      border: Border.all(
        color: Colors.white.withValues(alpha: dark ? 0.08 : 0.7),
        width: 1,
      ),
      boxShadow: [
        BoxShadow(
          color: dark
              ? Colors.black.withValues(alpha: 0.3)
              : const Color(0x14000000),
          blurRadius: 16,
          spreadRadius: -4,
          offset: const Offset(0, 6),
        ),
      ],
    );
  }

  /// 強調カード（クーポン・CTAなど）。ほのかなグラデを持つ。
  static BoxDecoration accentCard(
    BuildContext? context, {
    double radius = 22,
  }) {
    final dark = _isDark(context);
    return BoxDecoration(
      borderRadius: BorderRadius.circular(radius),
      gradient: LinearGradient(
        begin: Alignment.topLeft,
        end: Alignment.bottomRight,
        colors: dark
            ? const [Color(0xFF1B2233), Color(0xFF141A27)]
            : const [Color(0xFFFFFFFF), Color(0xFFF0F4FF)],
      ),
      border: Border.all(
        color: Colors.white.withValues(alpha: dark ? 0.08 : 0.8),
        width: 1,
      ),
      boxShadow: [
        BoxShadow(
          color: dark
              ? Colors.black.withValues(alpha: 0.4)
              : const Color(0x1F2E7CF6),
          blurRadius: 30,
          spreadRadius: -10,
          offset: const Offset(0, 14),
        ),
      ],
    );
  }
}
